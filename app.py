...
from . import app, db  # Importing app and db from __init__.py
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if 'Authorization' in request.headers:
            ...
            # Ensure token is in the correct format and strip 'Bearer ' prefix
            token = token.replace("Bearer ", "").strip()
            print(f"Token received: {token}")  # Debugging print to check the token
        ...
        try: 
            # Ensure correct decoding of the token
            data = jwt.decode(token, "secret", algorithms=["HS256"])
            current_user = Users.query.filter_by(id=data["id"]).first()
            print(f"Current user: {current_user}")  # Debugging to check user
